...
| <a name="input_authorizer_configuration"></a> [authorizer\_configuration](#input\_authorizer\_configuration) | Optional inbound CUSTOM\_JWT authorizer. When set, AgentCore validates the<br/>request's bearer JWT (issuer/audience/clients) against discovery\_url before<br/>it reaches the container. Leave null for SigV4 (IAM) inbound auth. | <pre>object({<br/>    discovery_url    = string<br/>    allowed_audience = optional(list(string))<br/>    allowed_clients  = optional(list(string))<br/>    allowed_scopes   = optional(list(string))<br/>  })</pre> | `null` | no |
...
| <a name="input_request_header_allowlist"></a> [request\_header\_allowlist](#input\_request\_header\_allowlist) | Inbound request headers AgentCore forwards to the container (e.g. ["Authorization"] so the runtime can re-verify the bearer). Empty = forward none. | `list(string)` | `[]` | no |
